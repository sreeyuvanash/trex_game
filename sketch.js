...
function setup() {
 ...
 console.log(message)
  ...
  bac2=createSprite(windowWidth/2,windowHeight/2,windowWidth,windowHeight)
  bac2.visible=false;
...
function draw() {
  ...
  if(gameState === PLAY){
   ...
   ground.velocityX = -(3*5 + score/200);
    //scoring
    ...
    if(keyDown("space")&& trex.y >= 270) {
      ...
      if(keyDown("up")&& trex.y >= 270) {
        trex.velocityY = -10.5;
        jumpSound.play();
    }
    if(mouseIsPressed===true&& trex.y >= 270) {
     trex.velocityY = -10.5;
        jumpSound.play();
    }
...
function spawnObstacles(){
 if (frameCount % 30 === 0){
   var obstacle = createSprite(windowWidth,295,10,40);
   obstacle.velocityX = -(3*5 + score/200);
